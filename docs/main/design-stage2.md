...
| Провайдер | Нативный режим | Что гарантируется | Адаптер |
|---|---|---|---|
| Gemini | `responseSchema` ([документация](https://ai.google.dev/gemini-api/docs/structured-output)) | соответствие схеме | готов |
| Claude | structured outputs: схема в запросе либо через определение tool ([документация](https://platform.claude.com/docs/en/build-with-claude/structured-outputs)) | соответствие схеме | отложен |
| DeepSeek | `response_format: {"type":"json_object"}` ([документация](https://api-docs.deepseek.com/guides/json_mode)) | только валидный JSON; схема не проверяется, её нужно описать в промпте | отложен |

Адаптеры Claude и DeepSeek отложены: ключей для них нет, а непроверенный код против чужого API — это код, который скорее всего не работает. Провайдер без адаптера **не попадает в конфигурацию**, даже если ключ задан: иначе интерфейс предложил бы выбор, который не работает. О найденном ключе без адаптера сообщается при старте.

### 3.2.1 Формат Gemini, сверенный живыми запросами

Документация не покрывает несколько деталей, каждая из которых ломала запрос:

- Путь `POST /v1beta/models/{model}:generateContent`, ключ в заголовке `x-goog-api-key`, а не в строке запроса — так он не попадёт в логи прокси.
- Типы в схеме пишутся **заглавными**: `OBJECT`, `STRING`, `NUMBER`, `BOOLEAN`, `ARRAY`.
- `null` выражается полем `nullable: true`, а не списком типов, как в JSON Schema.
- **Пустая строка в `enum` запрещена**: API отвечает `enum[0]: cannot be empty`. Поэтому «модель не нашла значение» выражается через `null`, и поля с наборами значений сделаны nullable. Это изменило внутреннюю схему из п. 6.3.
- `additionalProperties` не поддерживается и не отправляется.
- В `usageMetadata` кроме `promptTokenCount` и `candidatesTokenCount` есть **`thoughtsTokenCount`** — токены размышлений. Они тарифицируются как выходные, поэтому попадают в счётчик выходных: на живом запросе это 133 токена размышлений против 52 токенов ответа, и без их учёта оценка стоимости врала бы втрое.
- `thinkingConfig: {thinkingBudget: 0}` отключает размышления и на извлечении данных не портит результат — проверено сравнением ответов. Поэтому запрос идёт с отключёнными размышлениями, а если модель это запрещает (обычно pro-семейство), адаптер повторяет запрос без этой настройки.
- `gemini-2.5-pro` и `gemini-2.5-flash-lite` новым ключам больше не выдаются («no longer available to new users»), а pro-модели на бесплатном тарифе упираются в квоту. Поэтому по умолчанию предлагаются `gemini-2.5-flash` и `gemini-flash-latest` — обе проверены живым вызовом.
...
backend/internal/
  ...
  fetcher/              скачивание страницы (cookie jar, лимиты) и очистка HTML парсером
...
Разбор идёт настоящим парсером `golang.org/x/net/html`, а не регулярными выражениями: разметка на живых сайтах сломана в самых неожиданных местах, а парсер разбирает её как браузер и корректно отбрасывает содержимое `script` и `style`. Это единственная новая зависимость этапа.

Выбрасываются `script`, `style`, `noscript`, `svg`, `template`, `iframe`, `form`, `select`, а также `nav`, `footer` и `aside` — меню, подвал и блоки «похожие вакансии», из которых модель начинает собирать чужие данные. `header` оставлен: там обычно название должности.

В текст, помимо содержимого страницы, попадают три вещи с высоким сигналом:

- `<title>` — там почти всегда должность;
- `description` или `og:description` из мета-теги — у сайтов вакансий там нередко город и условия одной строкой;
- блоки `application/ld+json` с разметкой `JobPosting` (schema.org), если они есть: это самые точные данные на странице. Разметка хлебных крошек и организации отбрасывается как шум.

Порядок частей важен: заголовок и описание идут первыми, потому что при обрезке по лимиту сохраняется начало.

Объём результата сохраняется в `source_chars`. Ниже 500 символов появляется предупреждение о том, что страница, вероятно, рендерится на JavaScript или защищена от автоматических запросов; отдельное предупреждение — при совпадении с признаками страницы проверки браузера («Antibot», «captcha», «включите JavaScript»). Запуск при этом не блокируется, по принятому решению.

### 6.2.1 Скачивание

Фетчер использует **банку cookie на каждый запрос**. Это не про слежку, а про возможность вообще получить страницу: часть сайтов сначала ставит cookie и редиректит обратно, и без их сохранения запрос уходит в бесконечный цикл перенаправлений. Проверено на `ozon.tech`: без cookie — непонятная ошибка «цикл редиректов», с cookie — честный `403`. Банка своя на каждый запрос, чтобы cookie одного сайта не переживали до следующего скачивания.

Ограничения: пять перенаправлений, пять мегабайт тела, `Content-Type` только `text/html`, `application/xhtml+xml` или `text/plain` (PDF и картинки отправлять в модель бессмысленно, а токены они сожгут). Отсутствие заголовка `Content-Type` считается HTML: отказ был бы строже, чем нужно.

Виды неудач различаются явно — `invalid_url`, `network`, `timeout`, `http_status` (с кодом), `content_type` — чтобы журнал запусков показывал причину, а не общее «не получилось».
...
**Task 7. Адаптер Gemini**
Нативный структурированный вывод, счётчики токенов с учётом размышлений, реестр адаптеров, отключение размышлений с откатом. Тесты на стабах: в запрос уходит схема заглавными типами, ключ только в заголовке, `429`/`404`/`5xx` дают понятные сообщения, обрезанный ответ распознаётся.
*Demo:* на странице `rabota.sber.ru` извлечены должность, компания, грейд, шесть технологий и дата публикации, а вилка вернулась `null` — на странице цифр нет, и правило «не выдумывай» сработало.

**Task 11 выполнен в урезанном виде.** Диалог выбора провайдера не делался: настроен один провайдер, и выбирать не из чего. Кнопка сразу берёт его и модель по умолчанию, а предпросмотр с галочками сделан полностью — он и есть суть задачи. Диалог выбора понадобится, когда появится второй провайдер, то есть вместе с Task 8.

**Task 8. Адаптеры Claude и DeepSeek — отложено**
Ключей для проверки нет. Писать адаптер к чужому API без возможности выполнить запрос — значит закладывать код, который почти наверняка не работает. Реестр адаптеров и список реализованных провайдеров уже готовы, добавление сводится к одному файлу и одной строке в списке; сверку списков держит тест `TestRegistryCoversImplementedProviders`.
