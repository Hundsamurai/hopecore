...
Статус: реализовано. Все 12 задач из п. 11 выполнены, решения по ходу работы внесены в документ. Инструкция по запуску — в [`README.md`](../../README.md).
...
При открытии БД включаются `foreign_keys=on` (иначе SQLite игнорирует FK и каскадное удаление), `journal_mode=DELETE` и `busy_timeout=5000`.

**Про журнал.** Изначально был выбран `WAL`, но во время работы над Этапом 2 файл БД один раз оказался повреждён: после остановки контейнера главный файл был заполнен нулями, а `-wal` и `-shm` исчезли. Воспроизвести не удалось — при одновременной записи с хоста и из контейнера SQLite корректно отдаёт `database is locked`, а `integrity_check` проходит. Но картина отказа характерна для WAL на виртуализированной файловой системе: WAL держит состояние в разделяемой памяти через файл `-shm` и mmap, а файл БД лежит в bind-mount Docker Desktop и по замыслу доступен ещё и с хоста.

Решение: отказаться от WAL. Его смысл — параллельные читатели при активном писателе, а у нас одно соединение и один пользователь, так что выигрыша нет вообще. Откатный журнал разделяемую память не использует и убирает этот класс отказов целиком. Проверено десятью циклами «запись через API → чтение файла с хоста → перезапуск контейнера»: потерь нет, `-shm` не появляется, `integrity_check` проходит.

Дополнительно ошибка открытия испорченного файла переводится в понятный текст: голое `file is not a database (26)` выглядит как ошибка конфигурации, хотя означает повреждённый или посторонний файл.
...
Задачи идут инкрементально: каждая заканчивается проверяемым состоянием, а не «полуготовым слоем». Все выполнены; ниже сохранён исходный план с критериями приёмки.

Два отклонения от порядка задач по ходу работы: `activity.Resolve` сделан в Task 4, потому что без него не собрать фильтр `include_inactive`; минимальная карточка `/vacancies/:id` появилась в Task 8, иначе клик по строке таблицы вёл бы в «страницу не найдена».
...
Четыре gorm-модели, кастомный тип `Tags` (JSON ↔ `[]string`), `store.Open()` + `AutoMigrate`, включение `foreign_keys=on` и откатного журнала (изначально был `WAL`, заменён — см. п. 5.4). Тесты: миграция на in-memory БД создаёт все таблицы; round-trip `Tags` сохраняет порядок и корректно обрабатывает пустой список.
...
Настройки вынесены в `.env` через подстановку в `docker-compose.yml` (`HOST_PORT`, `DATA_DIR`, `CHECK_TIMEOUT`, `CHECK_CONCURRENCY`, `LOG_LEVEL`). Файл необязателен: без него работают значения по умолчанию, поэтому запуск после клонирования не требует настройки.
